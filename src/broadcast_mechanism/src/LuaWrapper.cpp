...
double LuaInterpreter::getNumber(int index)
{
    ...
    if (!isNumber)
    {
        LuaException e(" not a number at index " + 
                       std::to_string(index));
        throw e;
    }
...
string LuaInterpreter::getString(int index)
{
    ...
    if (stringPtr == NULL) {
        LuaException e(" not a string at index " +
                       std::to_string(index));
        throw e;
    }
...
void LuaInterpreter::pushMessage(Message& m)
{
    ...
    lua_pushstring(luaState_, m.messageId.c_str());
    lua_settable(luaState_, -3);

    //add messageBuffer
    lua_pushstring(luaState_, "buffer");
    lua_pushlstring(luaState_, &m.buffer[0], m.buffer.size());
    lua_settable(luaState_, -3);
}

void LuaInterpreter::pushMessages(vector<Message> messages)
{
